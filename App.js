...
import { StyleSheet, View, Text, StatusBar, Animated, Easing, Dimensions } from 'react-native';
...
export default function App() {
  ...
  const [turnInstruction, setTurnInstruction] = useState("CALIBRATING SENSORS...");
  ...
  useEffect(() => {
    ...
    const fusionLoop = setInterval(() => {
      ...
      let fusedHeading = (euler.heading * (180 / Math.PI) + 90 + 360 + 13.0 + GLOBAL_YAW_OFFSET) % 360;
    ...
    Animated.loop(
      Animated.timing(ghostAnimation, { toValue: 1, duration: 11000, easing: Easing.linear, useNativeDriver: true })
    ).start();
  ...
  const ghostX = ghostAnimation.interpolate({ inputRange: [0, 1], outputRange: [-200, width + 200] });
  const ghostY = ghostAnimation.interpolate({ inputRange: [0, 0.5, 1], outputRange: [height * 0.2, height * 0.1, height * 0.2] });
  const ghostOpacity = ghostAnimation.interpolate({ inputRange: [0, 0.1, 0.9, 1], outputRange: [0, 0.6, 0.6, 0] });

  return (
    <View style={styles.container}>
      <StatusBar hidden />
      <View style={StyleSheet.absoluteFill}><CameraView style={{ flex: 1 }} facing="back" active={true} /></View>
      
      <Animated.View style={[ styles.ghostContainer, { transform: [{ translateX: ghostX }, { translateY: ghostY }], opacity: ghostOpacity }, ]} pointerEvents="none" >
      ...
      <View style={styles.headerBar}>
        ...
        <View style={styles.signalContent}>
          ...
          <Text style={styles.signalDist}>{distFeet} FT // {distMeters} M</Text>
        </View>
      </View>

      <View style={styles.compassPosition}>
        <View style={[styles.ring, { transform: [{ rotate: `${(360 - safeFlatHeading) % 360}deg` }] }]}>
          <View style={styles.northMarker}><Text style={styles.nText}>N</Text></View>
        </View>
      ...
      <View style={styles.wayfinderLayer} pointerEvents="none">
        <View style={styles.compassBase}>
          ...
          <View style={[styles.floatingDisc, { transform: [{ rotate: `${safeWayfinderRot}deg` }] }]}>
      ...
      {safeDist < 25 && safeDist > 0 && (
        <View style={styles.infoPanel} pointerEvents="none">
          ...
          <Text style={styles.infoMeta}>{activeTarget.year} | {activeTarget.architect}</Text>
...
const styles = StyleSheet.create({
  ...
  load: { flex: 1, backgroundColor: '#000', justifyContent: 'center', alignItems: 'center' },
  ghostContainer: { position: 'absolute', width: 250, height: 250, zIndex: 10000, justifyContent: 'center', alignItems: 'center' },
  ghostSymbol: { fontSize: 120, color: 'rgba(0, 255, 255, 0.4)' },
  ghostText: { position: 'absolute', bottom: 30, color: 'rgba(0, 255, 255, 0.6)', fontSize: 9, fontWeight: '900', letterSpacing: 2 },
  headerBar: { position: 'absolute', top: 50, left: 20, width: '65%', backgroundColor: 'rgba(0,0,0,0.6)', padding: 15, borderLeftWidth: 4, borderLeftColor: '#00ffff', zIndex: 10 },
  headerLabel: { color: '#00ffff', fontSize: 8, fontWeight: '900', letterSpacing: 1.5, marginBottom: 5 },
  signalSub: { color: 'rgba(255,255,255,0.7)', fontSize: 9, fontWeight: 'bold' },
  signalName: { color: '#fff', fontSize: 13, fontWeight: '900' },
  signalDist: { color: '#fff', fontSize: 20, fontWeight: '300', marginTop: 2, letterSpacing: 1 },
  compassPosition: { position: 'absolute', top: 60, right: 30, zIndex: 10 },
  ring: { width: 60, height: 60, borderRadius: 30, borderWidth: 2, borderColor: '#00ffff', backgroundColor: 'rgba(0,0,0,0.8)', alignItems: 'center', justifyContent: 'center' },
  northMarker: { position: 'absolute', top: 2 },
  nText: { color: '#00ffff', fontSize: 14, fontWeight: '900' },
  fixedIndicator: { position: 'absolute', top: -4, left: 28, width: 4, height: 12, backgroundColor: '#ff3333', borderRadius: 2 },
  wayfinderLayer: { position: 'absolute', bottom: 100, alignSelf: 'center', alignItems: 'center', zIndex: 20 },
  compassBase: { width: 120, height: 120, borderRadius: 60, backgroundColor: 'rgba(0,0,0,0.8)', borderWidth: 1, borderColor: 'rgba(0,255,255,0.2)', justifyContent: 'center', alignItems: 'center', overflow: 'hidden' },
  lubberLine: { position: 'absolute', top: 0, width: 3, height: 12, backgroundColor: '#ff3333', zIndex: 10 },
  floatingDisc: { width: 100, height: 100, borderRadius: 50, justifyContent: 'center', alignItems: 'center' },
  targetIcon: { color: '#00ffff', fontSize: 28 },
  targetLabel: { color: '#00ffff', fontSize: 7, fontWeight: '900' },
  instructionPill: { marginTop: 15, backgroundColor: 'rgba(0,0,0,0.9)', paddingHorizontal: 20, paddingVertical: 8, borderRadius: 2, borderWidth: 1, borderColor: '#00ffff', alignItems: 'center' },
  instructionText: { color: '#fff', fontSize: 13, fontWeight: '900', letterSpacing: 1 },
  distanceText: { color: 'rgba(255,255,255,0.6)', fontSize: 9, marginTop: 2 },
  infoPanel: { position: 'absolute', bottom: 300, alignSelf: 'center', width: '85%', backgroundColor: 'rgba(0,0,0,0.95)', padding: 20, borderRadius: 2, borderWidth: 1, borderColor: '#00ffff', zIndex: 30 },
  infoTitle: { color: '#00ffff', fontSize: 16, fontWeight: '900', marginBottom: 5 },
  infoMeta: { color: '#fff', fontSize: 10, opacity: 0.6, marginBottom: 10, letterSpacing: 1 },
